...
function getSpiralMatrix(size) {
  ...
  let num = 1;
  while (step <= size / 2) {
    for (let j = step; j < size - step; j += 1) {
      arr[step][j] = num;
      num += 1;
    }
    for (let i = step + 1; i < size - step; i += 1) {
      arr[i][size - step - 1] = num;
      num += 1;
    }
    for (let j = size - step - 2; j >= step; j -= 1) {
      arr[size - step - 1][j] = num;
      num += 1;
    }
    step += 1;
    for (let i = size - step - 1; i >= step; i -= 1) {
      arr[i][step - 1] = num;
      num += 1;
    }
  }
